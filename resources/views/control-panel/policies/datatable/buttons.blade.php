...
<div class="modal fade" id="modals-edit-{{ $item->id }}">
    <div class="modal-dialog">
        ...
        <form class="create-new-nature modal-content pt-0"
            action=" {{ route('policies.update',$item->id) }} " method="POST" enctype="multipart/form-data">
             @csrf
             @method('put')
            <div class="modal-header mb-1">
                <h5 class="modal-title" id="exampleModalLabel">تعديل اللوائح والسياسات</h5>
            </div>
...
<div class="modal fade" id="modals-delete-{{ $item->id }}">
    <div class="modal-dialog">
        <form class="add-new-user modal-content pt-0" action=" {{ route('policies.destroy',$item->id) }}"
                method="POST">
                @csrf
                @method('DELETE')
            <div class="modal-header mb-1">
