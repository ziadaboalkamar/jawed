...
<div class="modal fade" id="modals-edit-{{ $item->id }}">
    <div class="modal-dialog course-model">
        ...
        <form class="create-new-nature modal-content pt-0"
            action=" {{ route('course-users.update',$item->id) }} " method="POST" enctype="multipart/form-data">
             @csrf
             @method('put')
...
<div class="modal fade" id="modals-delete-{{ $item->id }}">
    <div class="modal-dialog">
        <form class="add-new-user modal-content pt-0" action=" {{ route('course-users.destroy',$item->id) }}"
                method="POST">
                @csrf
                @method('DELETE')
            <div class="modal-header mb-1">
