...
<div class="modal fade" id="modals-edit-{{ $item->id }}">
    <div class="modal-dialog">
        ...
        <form class="create-new-nature modal-content pt-0"
            action=" {{ route('libraries.update',$item->id) }} " method="POST" enctype="multipart/form-data">
             @csrf
             @method('put')
            ...
            <div class="modal-body flex-grow-1">
                <div class="form-group">
                    <label for="username">اسم الملف</label>
                    <input type="text" class="form-control"
                        value="{{ $item->name }}" name="name" id="name" required>
                </div>
                <div class="form-group">
                    <label for="image">اضافة الملف</label>
...
<div class="modal fade" id="modals-delete-{{ $item->id }}">
    <div class="modal-dialog">
        <form class="add-new-user modal-content pt-0" action=" {{ route('libraries.destroy',$item->id) }}"
                method="POST">
                @csrf
                @method('DELETE')
            <div class="modal-header mb-1">
