...
<div class="modal fade" id="modals-edit-{{ $item->id }}">
    <div class="modal-dialog">
        ...
        <form class="create-new-nature modal-content pt-0"
            action=" {{ route('statistics.update',$item->id) }} " method="POST" enctype="multipart/form-data">
             @csrf
             @method('put')
            ...
            <div class="modal-body flex-grow-1">
                <div class="form-group">
                    <label for="username">الاحصائية</label>
                ...
                <div class="form-group">
                    ...
                    <input type="text" class="form-control"
                        value="{{ $item->number }}" name="number" id="number" required>
                </div>
...
<div class="modal fade" id="modals-delete-{{ $item->id }}">
    <div class="modal-dialog">
        <form class="add-new-user modal-content pt-0" action=" {{ route('statistics.destroy',$item->id) }}"
                method="POST">
                @csrf
                @method('DELETE')
            <div class="modal-header mb-1">
                <h5 class="modal-title" id="exampleModalLabel">حذف الاحصائية</h5>
            </div>
            <div class="modal-body flex-grow-1">
                <h4>هل تريد حذف الاحصائية؟</h4>
            </div>
            <div class="modal-footer">
